...
pub fn load_token_server_config() -> Option<TokenServerConfig> {
    let mut config: TokenServerConfig = serde_json::from_str(TOKEN_SERVER_CONFIG).ok()?;
    
    // In test environment, allow using local dev server
    #[cfg(feature = "dev-server")]
    {
        config.host = String::from("http://localhost:3000");
    }
...
pub fn token_server_url(endpoint: &str) -> Option<String> {
    ...
    let base = config.host.trim_end_matches('/');
...
mod token_server_tests {
    ...
    fn config_has_valid_host() {
        ...
        assert!(
            config.host.starts_with("http://") || config.host.starts_with("https://")
        );
        assert_eq!(config.host, get_expected_host());
    }
